...
def train():
    ...
    #首先计算隐藏层，就是简单地embedding一下，把输入的各个整数（每个整数代表一个单词）变为一个向量。
    hidden_ = computeHidden(train_data_node)
    #分层soft_max的计算。解决了当分类太多时计算量过大的问题。fasttext中使用的就是此技术。
    ...
    with tf.Session() as sess:
        ...
        for batch in batches:
            ...
            feed_dict = {train_data_node1: x_batch[0],train_labels_node: y_batch[0],train_label_num_node:label_num_batch[0],label_path_length_node:path_length_batch[0]}
